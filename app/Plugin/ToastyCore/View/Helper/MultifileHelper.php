...
class MultifileHelper extends AppHelper {
    ...
    function makeFileArea($elementName, $formName, $maxFiles = 3, $valid = '*') {
        ...
        $css = $this->Html->css('ToastyCore.multifile');
        $camelized = Inflector::camelize($elementId);
        $script = $this->Html->script('ToastyCore.multifile') . "<script> var valid = $line; var multi_selector_$elementName = new MultiSelector( document.getElementById( '${elementName}_list' ), $maxFiles, '" . ucfirst($formName) . "', '$elementName', valid ); multi_selector_$elementName.addElement( document.getElementById( '$camelized' ) );</script>";
